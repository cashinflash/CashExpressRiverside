# apply.cashexpressriverside.com

Backend + frontend for the Cash Express Riverside application funnel.
Fork of `cashinflash/cif-apply` main branch with:

- Frontend rebranded (navy + sunglow yellow, Cash Express Riverside logo + copy).
- Backend hardcodes `source_site='cashexpress_riverside'` on every `/submit`.
...
| Domain | No | `apply.cashexpressriverside.com` (independent of `apply.cashinflash.com`). |

## Deployment on Render

See the step-by-step in the marketing-site repo's README. Short version:

1. Create a new **Project** in Render called `Cash Express Riverside`.
2. In that project, create a **Web Service** pointed at
   `cashinflash/CashExpressRiverside` with **Root Directory** set to `apply`.
3. Copy env vars from your existing `cif-apply` service into the new service
   (use `.env.example` above as a checklist).
4. Deploy.
5. Add custom domain `apply.cashexpressriverside.com` in Render's domain
...
The `/submit` tagging lines are Riverside-specific and should NOT be
